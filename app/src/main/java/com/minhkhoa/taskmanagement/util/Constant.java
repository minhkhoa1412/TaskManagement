package com.minhkhoa.taskmanagement.util;

public class Constant {
    public static final String BOARD_MAIN_TO_LIST = "BOARD_MAIN_TO_LIST";
    public static final String BOARD_ID = "ID_BOARD";
    public static final String POSITION_BOARD = "POSITION_BOARD";
    public static final String BOARD_NAME = "BOARD_NAME";
    public static final String BUNDLE_MAIN_TO_LIST = "BUNDLE_MAIN_TO_LIST";
    public static String COLLECTION_ID = "327760";
    public static final String LIST_ID = "LIST_ID";
    public static final String LIST_NAME = "LIST_NAME";
    public static final String BUNDLE_LIST_TO_CARD = "BUNDLE_LIST_TO_CARD";
    public static final String CARD_ARRAY_DETAILS = "CARD_ARRAY_DETAILS";
    public static final String CARD_FRAGMENTS = "CARD_FRAGMENTS";
    public static final String BUNDLE_CARD_TO_DETAILS = "BUNDLE_CARD_TO_DETAILS";
    public static final String BOARD_CHAT = "BOARD_CHAT";
    public static final String BUNDLE_LIST_TO_CHAT = "BUNDLE_LIST_TO_CHAT";
    public static final String CHAT_FRAGMENTS = "CHAT_FRAGMENTS";
    public static final String BOARD_ID_FOR_CHAT_CARD = "BOARD_ID_FOR_CHAT_CARD";
    public static final String USER_TO_BOARD_INVITED = "USER_TO_BOARD_INVITED";
    public static final String CHAT_CHANNEL_ID = "CHAT_CHANNEL_ID";
}
